...
st.markdown("""
    ...
    .ticker-wrap { width: 100%; overflow: hidden; background: #000; color: #d4af37; padding: 8px 0; font-family: 'Arial Black'; border-bottom: 2px solid #d4af37; }
    .ticker-content { display: inline-block; white-space: nowrap; animation: ticker 60s linear infinite; }
    .ticker-item { display: inline-block; margin-right: 80px; font-size: 16px; }
    </style>
...
if not st.session_state.entered:
    ...
    st.markdown("<h1 style='font-size: 60px; color: #d4af37; margin: 0;'>SPAM LEAGUE</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: white; letter-spacing: 4px;'>COMMISSIONER DATA TERMINAL</p>", unsafe_allow_html=True)
...
# 4. MAIN INTERFACE (RE-ENABLE SCROLLING)
...
if p_avg is not None:
    ...
    with tabs[0]: # CLICKABLE SCOUTING
        st.subheader("Select Player for Scouting Report")
        ...
        if len(sel.selection.rows) > 0:
            ...
            with c1: st.metric("PTS HIGH", int(hist['PTS'].max())); st.metric("LAST GAME", int(hist.iloc[0]['PTS']))
            with c2: st.line_chart(hist.set_index('Game_ID')['PTS'], height=180)

    with tabs[1]: # STANDINGS (KEYERROR FIX)
    ...
    with tabs[2]: # TOP 10 LISTS
        cat = st.selectbox("Category", ["PTS/G", "REB/G", "AST/G", "PIE"])
        t10 = p_avg.nlargest(10, cat)[['Player/Team', 'Team Name', cat]].reset_index(drop=True)
        t10.index += 1
        st.table(t10)
        st.plotly_chart(px.bar(t10, x=cat, y='Player/Team', orientation='h', template="plotly_dark"), use_container_width=True)

    with tabs[4]: # RECORDS
        r_p = df_raw.loc[df_raw['PTS'].idxmax()]
        st.metric("All-Time Scoring Record", int(r_p['PTS']), r_p['Player/Team'])
