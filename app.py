...
if isinstance(full_df, str): 
    ...
    view_mode = st.sidebar.radio("Navigation", ["🏠 League Home & Awards", "🏆 Power Rankings & SOS", "🏢 Team Pages", "🗃️ Full Player Database", "⚔️ Rivalry Corner", "🔮 Oracle Predictor", "🔬 Advanced Analytics", "🏦 The Vault"])
    st.sidebar.divider()
    ...
    if view_mode == "🏠 League Home & Awards":
        ...
        # Hot/Cold logic based on chronological game IDs
        ...
        if 'Opp_Name' in matchups.columns:
            # Create a unique sorted pairing string to group A vs B and B vs A together
        ...
        with c2:
            ...
            fig.update_yaxes(autorange="reversed") # Lower Opp PPP is better
        ...
        if not t_stats.empty and len(t_stats) >= 2:
            ...
            if st.button("Simulate Matchup"):
                ...
                t2_adj_def = d2['Opp_PPP'] * 100 # Rough scale
                ...
                def generate_sim_box(team_name, total_pts):
                    ...
                    for i, (_, p) in enumerate(roster.iterrows()):
                        if i == len(roster) - 1: share = pts_pool # Last guy gets remainder
