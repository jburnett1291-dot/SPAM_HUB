...
# 2. DATA ENGINE (FIXED FOR KEYERROR)
...
def load_data():
    try:
        ...
        # Force essential columns to exist as numeric
        ...
        # Team Standings (Ensuring 'Win' is kept for sorting)
...
if p_avg is not None:
    ...
    with tabs[1]: # STANDINGS (ERROR-FIXED LINE)
        # We check if t_stats is valid and sort by 'Win' which is now guaranteed to exist
    ...
    with tabs[3]: # VERSUS
        ...
        if v_mode == "Player vs Player":
            ...
            for s in ['PTS_Avg', 'REB_Avg', 'AST_Avg', 'STL_Avg', 'BLK_Avg']:
                ...
                sc2.metric(f"{t2} {s.split('_')[0]}", td2[s], delta=round(td2[s]-td1[s], 1))
