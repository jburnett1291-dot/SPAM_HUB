...
def show_card(name, stats_df, raw_df, is_player=True):
    ...
    h = st.columns(5)
    h[0].metric("Max PTS", int(personal['PTS'].max() if not personal.empty else 0))
    h[1].metric("Max REB", int(personal['REB'].max() if not personal.empty else 0))
    h[2].metric("Max AST", int(personal['AST'].max() if not personal.empty else 0))
    h[3].metric("Max STL", int(personal['STL'].max() if not personal.empty else 0))
    h[4].metric("Max BLK", int(personal['BLK'].max() if not personal.empty else 0))
    st.markdown("---")
    ...
    for _, g in recent.iterrows():
        ...
        if g['is_ff']: st.info(f"{label} - FORFEIT")
            ...
            f[3].metric("STL", int(g['STL'])); f[4].metric("BLK", int(g['BLK'])); f[5].metric("FG%", f"{g['FG%_Raw']}%")
...
if isinstance(full_df, str):
    ...
    with tabs[3]: # VERSUS (FIXED FOR TEAM COMPARISON)
        ...
        if v_mode == "Player vs Player":
            ...
            p1 = v1.selectbox("T1", t_stats.index, index=0); p2 = v2.selectbox("T2", t_stats.index, index=1)
            d1, d2 = t_stats.loc[p1], t_stats.loc[p2]
            metrics = ['Record', 'PTS/G', 'REB/G', 'AST/G', 'TO/G', 'FG%', 'OffRtg']
        
        for s in metrics:
            c1, c2 = st.columns(2)
            if s == 'Record':
                c1.metric(f"{p1} {s}", d1[s]); c2.metric(f"{p2} {s}", d2[s])
            else:
    ...
    with tabs[5]:
        ...
        if st.text_input("Passcode", type="password") == "SPAM2026":
            ...
            if not adv.empty:
                ...
                st.markdown("---")
                v_view = st.selectbox("View", ["Vol vs Eff", "Eff Hub", "Poss Control", "Splits"])
