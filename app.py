...
if isinstance(full_df, str):
    ...
    with st.sidebar:
        ...
        selected_season = st.selectbox("View Season", seasons, index=0)
        st.divider()
        st.caption("Season 4 is currently live.")

    # Filter data for the active view
    df_active = full_df[full_df['Season'] == selected_season]
    ...
    st.markdown(f'<div class="header-banner">🏀 SPAM LEAGUE CENTRAL - SEASON {int(selected_season)}</div>', unsafe_allow_html=True)

    tabs = st.tabs(["👤 PLAYERS", "🏘️ STANDINGS", "🔝 LEADERS", "⚔️ VERSUS", "📖 ALL-TIME RECORDS"])
    ...
    st.markdown(f'<div style="text-align: center; color: #444; padding: 30px;">© 2026 SPAM LEAGUE HUB | CURRENTLY VIEWING SEASON {int(selected_season)}</div>', unsafe_allow_html=True)
